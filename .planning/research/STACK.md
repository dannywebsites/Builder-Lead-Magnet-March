...
**Domain:** Single-page financial calculator lead magnet (web app, no backend persistence)
...
```bash
# Core
pnpm create next-app@latest trade-survival-calculator --typescript --tailwind --app --use-pnpm

# Supporting
pnpm install @react-pdf/renderer react-hook-form @hookform/resolvers zod nuqs sonner
...
**If email volume exceeds 3,000/month (Resend free tier):**
...
**If SEO is deprioritized (e.g., traffic comes purely from paid ads / social):**
...
**If PDF generation becomes a performance bottleneck on mobile:**
...
**If you want to A/B test the calculator flow:**
...
The calculator itself is pure client-side: form inputs go through Zod validation, feed into deterministic calculation functions, and render results. No server needed for any of this.

The only server-side requirement is the email capture + delivery at report download. This is a single Next.js API route (`/api/capture`) that:
1. Receives: email address + calculation results JSON
2. Sends to Resend: branded results email
3. Optionally: forwards email to a marketing platform webhook (ConvertKit, Mailchimp) for nurture sequences
4. Returns: success/failure

This can be deployed as a Vercel Serverless Function (included free on Hobby plan) alongside the static export.

### Multi-Step Form Architecture

```
FormProvider (react-hook-form)
  -> Step 1: Entity Type + Market (UK/IE)
  -> Step 2: Personal Draw + Tax
  -> Step 3: Overheads + Staff
  -> Step 4: Job Pricing + Direct Costs
  -> Step 5: Results Display
  -> Step 6: Email Gate -> PDF Download
```

Each step is a React component. Form state persists across steps via react-hook-form's `FormProvider`. URL query parameter (`?step=3`) via nuqs allows back/forward navigation. Zod schema validates per-step with `schema.pick()`.

### Calculation Layer

All financial calculations live in pure TypeScript functions with zero dependencies — fully testable, no React imports. Input types and output types defined with Zod schemas. This separation means:
...
---
*Stack research for: Trade Survival Calculator (financial lead magnet for UK/IE tradespeople)*
*Researched: 2026-03-28*
